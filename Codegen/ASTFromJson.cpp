...
{
    if (json.at("type") == "Function") return std::make_unique<FunctionNode>(json, node_from_type(json.at("prototype")), create_AST(json.at("body")));
    ...
    if (json.at("type") == "Call") return std::make_unique<CallNode>(json, create_AST(json.at("args")));
    ...
    if (json.at("type") == "If") return std::make_unique<IfNode>(json, node_from_type(json.at("condition")), create_AST(json.at("then")), create_AST(json.at("else")));
    if (json.at("type") == "Loop") return std::make_unique<LoopNode>(json, create_AST(json.at("body")));
    ...
    if (json.at("type") == "StructInstance") return std::make_unique<StructInstanceNode>(json, create_AST(json.at("members")));
    if (json.at("type") == "StructDefinition") return std::make_unique<StructDefinitionNode>(json);

    error("Blabla error in node_from_type");
}
